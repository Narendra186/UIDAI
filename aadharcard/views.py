...
def download(request):
    if request.method == 'POST':
        ...
        cursor.execute("INSERT INTO verify_otp VALUES ({},{},{})".format(aadhar_num,phone_num,otp))
...
def aadhar_card(request):
    if request.method == 'POST':
        ...
        cursor.execute("SELECT otp,aadhar_num FROM verify_otp WHERE phone_num={}".format(phone_num))
        real_otp = cursor.fetchone()
        print(real_otp[0])
        print(otp)
        if int(otp) == int(real_otp[0]):
            cursor.execute("DELETE FROM verify_otp WHERE phone_num={}".format(phone_num))
            ...
            cursor.execute('SELECT * FROM details WHERE aadhar_num={}'.format(560522624339))
            parse = cursor.fetchone()
            ...
            cursor.execute('SELECT * FROM pincodes WHERE pincode={}'.format(parse[12]))
            det = cursor.fetchone()
            y=y-18
            line8=det[1]
            ...
            cursor.execute('SELECT state FROM states WHERE id={}'.format(det[3]))
...
def find(request):
    if request.method == 'POST':
        ...
        cursor.execute("SELECT phone FROM details")
        phonee = cursor.fetchall()
        mobile = int(mobile)
        print(mobile)
        for phone in phonee:
            print(phone[0])
            if mobile == phone[0]:
                cursor.execute("SELECT first_name,middle_name,last_name FROM details WHERE phone={}".format(mobile))
                name = cursor.fetchone()
                full_name = name[2]+' '+name[0]+' '+name[1]
                print(full_name)
                print(fullname)
                if full_name == fullname:
                    ...
                    cursor.execute("INSERT INTO verify_otp (phone_num,otp) VALUES ({},{})".format(mobile,otp))
...
def otp_verify(request):
    if request.method == 'POST':
        ...
        mobile = int(mobile)
        otp = int(otp)
        cursor.execute("SELECT otp FROM verify_otp WHERE phone_num={}".format(mobile))
        num = cursor.fetchone()
        print(otp)
        print(num[0])
        if otp == num[0]:
            cursor.execute("SELECT aadhar_num FROM details where phone={}".format(mobile))
            uid = cursor.fetchone()
            smss = smsender()
            mes = "Your aadharcard number is"+' '+str(uid[0])
            smss.send_sms(mes,mobile)
            return HttpResponse("Your aadhar number is sent to your registered phone number")
        else:
